...
- **Codex tracker fix**: Codex CLI 0.147 (app-server mode) stopped writing `~/.codex/sessions/**/*.jsonl`, freezing the tracker at stale data (Aug 14). `CodexUsageProvider` now queries the live ChatGPT usage API (`GET https://chatgpt.com/backend-api/codex/usage?limit_id=codex`) with the OAuth token from `~/.codex/auth.json` — the same endpoint the CLI polls in-process. JSONL parsing remains as a fallback when the API is unreachable. Verified live: the user was at 100% used → 0% remaining (previously showed stale 4%).
...
- **Tests**: `CommandCodeUsageProviderTests` (6 tests: parsing, empty windows, plan name, missing auth, configured, plan display), `CodexUsageProviderTests` +6 API tests (API parse, at-limit, API-fail JSONL fallback, missing-auth fallback, configured-with-auth, loadAuthTokens); legacy JSONL tests now point at a nonexistent auth file so they don't hit the real API. Verified green: Codex (20), CommandCode (6), OpenCodeGo, UsageHistoryViewModel, UsageHistoryTabView, UsageViewModel, SettingsViewBehavior, DetailPopover, ServiceTypeColor, UsageHistoryStore. Full-suite run exceeds the 15-min xcodebuild budget on this machine; targeted suites all pass.
